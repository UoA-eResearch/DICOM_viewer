...
#pragma warning restore 0414
		...
		/// <param name="scale">scale relative to the shared anchor</param>
		...
		{
			target.GetComponent<SyncLocalTransformUNET>().RpcSetLocalTransform(pos, rot, scale);
		...
		public void SendSharedSavedPosition(GameObject target, Vector3 newPos, Quaternion newRot, Vector3 newScale)
		{
			if (isLocalPlayer) {
				CmdSendSharedSavedPosition(target, newPos, newRot, newScale);
			}
		}

		[Command(channel = Channels.DefaultUnreliable)]
		private void CmdSendSharedSavedPosition(GameObject target, Vector3 newPos, Quaternion newRot, Vector3 newScale)
		{
		...
		public void SendToggleGroup(GameObject target, int groupNumber)
		{
			if (isLocalPlayer)
			{
				CmdSendToggleGroup(target, groupNumber);
			}
		}

		[Command(channel = Channels.DefaultUnreliable)]
		private void CmdSendToggleGroup(GameObject target, int groupNumber)
		{
			target.GetComponent<SyncGenomicsUNET>().RpcToggleGroup(groupNumber);
		}
		...
		[Command(channel = Channels.DefaultUnreliable)]
		private void CmdSendToggleLabels(GameObject target, bool toggle)
		{
        ...
        public void SendToggleSamplingSites(GameObject target, bool active)
		{
			if (isLocalPlayer)
			{
				CmdSendToggleSamplingSites(target, active);
			}
		}

		[Command(channel = Channels.DefaultUnreliable)]
		private void CmdSendToggleSamplingSites(GameObject target, bool active)
		{
